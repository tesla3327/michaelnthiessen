...
Sometimes you just need to force Vue to update.

Or maybe you just want to blow away the current DOM and start over.

But how do you get Vue to re-render a component **the right way?**
...
The different methods I'm going to show you are still useful though. If you just need to get some code out, or hack together something that works (for now), these are great tools. But in the long run it's better for you and your code if you re-visit this and see if there is a different approach you can take.
...
There isn't really much more to say. Let's look for a better way.
...
2. When we call `forceRender` we immediately set `renderComponent` to `false`
...
Normally, Vue will react to changes in dependencies by updating the view. However, when you call `forceUpdate`, you can force that update to occur, even if nothing has actually updated.

So here is where most people make the **biggest mistakes** with this method.
...
So just like the last methods, if you need this to re-render your component, **you're probably doing something wrong**.
...
To do this the proper way, we will supply a `key` attribute so Vue knows that a specific component is tied to a specific object. If the key stays the same, it won't change the component, but if the key changes, Vue knows that it should **get rid of the old component and create a new one**.

Exactly what we need!

But first we'll need to take a *very short* detour.

For example, you're rendering a list of components that has one or more of the following:
...
Was that a short detour?

I'm not sure. I guess it depends on how you define "short".
